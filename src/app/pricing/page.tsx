...
import { ProgramPurchaseDialog } from "@/components/program-purchase-dialog";
...
const pricingCopy = {
  es: {
    ...
    plans: [
      { name: "Clases privadas", eyebrow: "Atención individual", price: "RD$900", cadence: "por hora", description: "Un plan uno a uno para trabajar su nivel, sus objetivos y las situaciones en las que realmente necesita comunicarse.", badge: "15 % de descuento el primer mes", icon: Target, features: ["Mínimo de 10 horas al mes", "Clases de 1 a 2 horas", "Compromiso inicial de 3 meses", "Plan y corrección personalizados"] },
      { name: "Clases grupales", eyebrow: "Programa por nivel", price: "RD$1,500", cadence: "por persona al mes", description: "Una rutina constante para aprender con estudiantes de nivel similar, practicar en grupo y avanzar por unidades.", badge: "15 % de descuento el primer mes", icon: GraduationCap, featured: true, features: ["Lunes y miércoles o martes y jueves", "Dos horas por clase", "Grupo organizado por nivel", "Interacción y práctica guiada"] },
      { name: "Tutoría personalizada", eyebrow: "Apoyo puntual", price: "RD$1,000", cadence: "por tutoría", description: "Apoyo específico para resolver una dificultad, preparar una asignación o practicar una situación concreta.", badge: "Modalidad flexible", icon: MessageCircle, features: ["Sesiones por día o por semana", "Enfoque en una necesidad concreta", "Práctica y explicación dirigidas", "Horario sujeto a disponibilidad"] },
    ],
...
export default async function PricingPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-surface-cream text-brand-navy">
      ...
      <section id="pricing-hero" className="relative isolate overflow-hidden bg-brand-navy px-4 py-14 text-white sm:px-6 sm:py-16 lg:px-8 lg:py-20"><div className="orbital-grid pointer-events-none absolute inset-0 opacity-10" /><div className="program-ring pointer-events-none absolute -right-48 top-10 hidden size-[520px] rounded-full opacity-70 blur-2xl md:block" /><div className="mx-auto max-w-4xl text-center"><MotionReveal><p className="section-kicker-dark">{copy.heroKicker}</p><h1 className="section-heading mx-auto mt-4 max-w-4xl text-white">{copy.heroTitle}</h1><p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-white/72 sm:text-lg sm:leading-8">{copy.heroBody}</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Link href={localePath(locale, "/contact")} className="button-primary">{copy.heroCta} <CalendarCheck size={18} aria-hidden /></Link><a href="#planes" className="button-secondary">{copy.heroAnchor} <ArrowRight size={18} aria-hidden /></a></div></MotionReveal></div></section>
